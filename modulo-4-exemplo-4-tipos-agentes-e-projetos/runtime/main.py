...
def _resumir_trace(dados: dict) -> str:
    """Resumo compacto do trace — dados completos lidos de trace.json pelas ferramentas."""
    linhas = [
        f"TRACE_ID: {dados.get('trace_id', '?')}",
        ...
        "FONTE_DADOS: runtime/trace.json",
    ]
    ...
    for etapa in dados.get("etapas", []):
        ...
        ferramenta = plano.get("nome_ferramenta", "-") or plano.get("proxima_acao", "-")
        qualidade = etapa.get("avaliacao", {}).get("qualidade", "")
        linhas.append(f"ETAPA {num}: {ferramenta} qualidade={qualidade}")

    if dados.get("resumo"):
        linhas.append(f"RESUMO: {dados['resumo'][:200]}")

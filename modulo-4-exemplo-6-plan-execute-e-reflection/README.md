# Atividade: Plan-Execute e Reflection

Este diretório é o **Módulo 4 — Exemplo 6** (`modulo-4-exemplo-6-plan-execute-e-reflection`) — duas novas arquiteturas cognitivas no mesmo slot da aula 7.

Referência UNIPDS: [aula08-plan-execute-e-reflection](https://github.com/unipds-engenharia-de-ia-aplicada/engenharia-de-software-com-ia-aplicada/tree/main/modulo04-agentes-autonomos/aula08-plan-execute-e-reflection)

## Objetivo

Comparar **ReAct** (passo a passo), **Plan-Execute** (plano completo na 1ª etapa) e **Reflection** (crítica antes de finalizar) — trocando apenas `--arquitetura`, sem alterar o `monitor-agent`.
...
|---------|-----|
| **Python 3.10+** | Runtime |
| **OpenRouter** | `OPENROUTER_API_KEY` no `runtime/.env` (reutilize do Ex. 5) |
| **Exemplo 5** | Slot `--arquitetura` e ReAct já validados |

## Configuração

```bash
cd modulo-4-exemplo-6-plan-execute-e-reflection/runtime
cp .env.example .env
pip install -r requirements.txt
```

## Passo a passo (aula08)

```bash
# ReAct (baseline da aula 7)
python main.py rodar --agente ../monitor-agent --entrada "alerta de latencia no servico de pagamentos" --arquitetura react

# Plan-Execute — plano completo na etapa 1, tokens=0 nas seguintes
python main.py rodar --agente ../monitor-agent --entrada "alerta de latencia no servico de pagamentos" --arquitetura plan_execute

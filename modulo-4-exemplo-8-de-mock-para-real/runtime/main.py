...
from benchmark import rodar_benchmark, comparar_todas
...
def main():
    ...
    # benchmark
    parser_benchmark = subparsers.add_parser("benchmark", help="Roda benchmark de uma arquitetura contra uma eval suite")
    parser_benchmark.add_argument("--agente", required=True, help="Caminho para a pasta do agente")
    parser_benchmark.add_argument("--suite", required=True, help="Caminho para o YAML da eval suite")
    parser_benchmark.add_argument("--arquitetura", required=False, help="Arquitetura cognitiva (react, plan_execute, reflect)")

    # comparar
    parser_comparar = subparsers.add_parser("comparar", help="Compara as 4 arquiteturas e gera report.md")
    parser_comparar.add_argument("--agente", required=True, help="Caminho para a pasta do agente")
    parser_comparar.add_argument("--suite", required=True, help="Caminho para o YAML da eval suite")
    ...
    if argumentos.comando == "rodar":
        ...
        pasta_bench = Path(__file__).parent.parent / "benchmarks"
        pasta_bench.mkdir(exist_ok=True)
        nome = getattr(argumentos, "arquitetura", None) or "padrao"
        caminho_json = pasta_bench / f"bench_{nome}.json"
        caminho_json.write_text(json.dumps(resultado, indent=2, ensure_ascii=False), encoding="utf-8")
        print(f"  Resultado salvo: {caminho_json}")
    elif argumentos.comando == "comparar":
        pasta_bench = Path(__file__).parent.parent / "benchmarks"
        comparar_todas(
            caminho_agente=argumentos.agente,
            caminho_suite=argumentos.suite,
            pasta_saida=str(pasta_bench),
        )
    else:
        parser.print_help()

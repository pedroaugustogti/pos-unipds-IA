  ...
  return async (state: GraphState): Promise<Partial<GraphState>> => {
    ...
    try {
      ...
      if (data?.requiresDecomposition && !!data.subQuestions?.length) {
        const subQuestions = data.subQuestions.slice(0, config.maxSubQuestions);
        const subQuestionsFormatted = subQuestions
          .map((q: string, i: number) => `\n   ${i + 1}. ${q}`)
          .join('');

        console.log(`📊 Complex query - ${subQuestions.length} steps:${subQuestionsFormatted}`);

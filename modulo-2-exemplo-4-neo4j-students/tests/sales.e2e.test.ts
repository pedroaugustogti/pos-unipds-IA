...
describe('Sales Analytics Reporter - E2E Tests', { timeout: TEST_TIMEOUT_MS * 12, concurrency: 1 }, () => {
    ...
    it('List all courses - Should return analytical response', { timeout: TEST_TIMEOUT_MS }, async () => {
    ...
    it('Query students who bought specific course - Should provide details', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Query total revenue from credit cards - Should include analytics', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Revenue distribution analysis - Should show percentages and comparisons', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Query students with 100% progress - Should provide insights', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Payment method analysis - Should compare different methods', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Edge case: Students with progress but no purchase - Should handle gracefully', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Invalid/unclear question - Should handle gracefully with suggestions', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Query students who never started a course - Should show engagement gaps', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(
    ...
    it('Follow-up questions - Should provide relevant suggestions', { timeout: TEST_TIMEOUT_MS }, async () => {
        const response = await makeASalesRequest(

...
def _modo_planejador() -> str:
    """auto = mock quando todas as ferramentas sao deterministicas; llm = forcar LLM."""
...
def _ordem_ferramentas(contratos: dict) -> list:
    ...
    # trace-analyzer: todas as skills sao obrigatorias — seguir ordem de rules.md
    ...
    # monitor-agent: ordem natural de coleta em skills.md; obrigatorias checadas ao FINALIZAR
...
def chamar_llm(percepcao: str, contratos: dict, historico: list = None, entrada: str = "") -> tuple:
    ...
    Quando todas as ferramentas tem implementacao deterministica, usa planejador_mock (0ms).
    """
    historico = historico or []

    if _deve_usar_planejador_mock(contratos):
        return planejador_mock(percepcao, contratos, historico, entrada), _TOKENS_ZERO.copy()
    ...
    try:
        ...
        return planejador_mock(percepcao, contratos, historico, entrada), uso_tokens
...
def planejador_mock(percepcao: str, contratos: dict, historico: list = None, entrada: str = "") -> dict:
    ...
    # descobre qual a proxima ferramenta nao usada (via historico, nao substring na percepcao)
    for nome in ordem:

# Atividade: Fine-Tuning via API

Este diretório é o **Módulo 9 — Exemplo 3** (`modulo-9-exemplo-3-fine-tuning-via-api`) — adaptação local da atividade da pós-graduação **Engenharia de IA Aplicada (UNIPDS)**.
...
Implementar Fine-Tuning via API conforme material UNIPDS Amplitude Seguros
...
```bash
cd modulo-9-exemplo-3-fine-tuning-via-api
...
- [ ] Pasta criada no padrão `modulo-9-exemplo-3-*`

...
Suporta arquiteturas cognitivas via formato_saida do contrato (ex.: ReAct).
"""
...
def construir_prompt_sistema(contratos: dict) -> str:
    ...
    if isinstance(formato_saida, dict) and formato_saida:
        campos_formato = [f'  "{campo}": "{descricao}"' for campo, descricao in formato_saida.items()]
        bloco_formato = "{\n" + ",\n".join(campos_formato) + "\n}"
    else:
        bloco_formato = """{
...
def _injetar_campos_arquitetura(plano: dict, contratos: dict, usadas: set) -> dict:
    """Preenche campos extras definidos pela arquitetura (ReAct, etc.)."""

...
describe('API Workflow', () => {
    ...
    describe('Auth & rate limit (no database seed)', () => {
        describe('POST /v1/auth/service-token', () => {
        const ADMIN_SUPER_SECRET = 'AM I THE BOSS?'

        it('should return role, department and a UUID serviceToken for valid admin credentials', async () => {
            ...
            deepStrictEqual(result.department, 'sales')
            ok(result.serviceToken)
            ok(/^[0-9a-f-]{36}$/.test(result.serviceToken), 'serviceToken should be a UUID')
        })

        it('should return role, department and a UUID serviceToken for valid member credentials', async () => {
            const res = await _testServer.inject({
                method: 'POST',
                url: `${_testServerAddress}/v1/auth/service-token`,
            ...
            deepStrictEqual(result.department, 'support')
            ok(/^[0-9a-f-]{36}$/.test(result.serviceToken), 'serviceToken should be a UUID')
        })
    ...
    describe('Service token - API access & rate limiting', () => {
        ...
        it(`should allow API access and rate limit after ${REQUESTS_PER_MINUTE} requests with the service token`, async () => {
            ...
            const rateLimited = await _testServer.inject(opts)
            deepStrictEqual(rateLimited.statusCode, 429)
        })
    })
    ...
    describe('CRUD & RBAC (with database seed)', () => {
        beforeEach(async () => {
            return runSeed()
        })
    ...
    describe('RBAC - member role', () => {
        ...
        it('member cannot create customer (POST /v1/customers)', async () => {
            ...
            ok((await res.json()).message.includes("role 'member'"))
        })

        it('member from support can update customer (PUT /v1/customers/:id)', async () => {
            const created = await createCustomer({ name: 'RBAC Test', phone: '000' })
            const { id } = await created.json()
        ...
        it('member from engineering cannot update customer (PUT /v1/customers/:id)', async () => {
            const loginRes = await _testServer.inject({
                method: 'POST',
                url: `${_testServerAddress}/v1/auth/login`,
                payload: { username: 'devuser', password: 'dev123' },
            })
            const { token } = await loginRes.json()
            const created = await createCustomer({ name: 'RBAC Test', phone: '000' })
            const { id } = await created.json()
            const res = await updateCustomer(id, { name: 'Changed', phone: '111' }, token)
            deepStrictEqual(res.statusCode, 403)
            ok((await res.json()).message.includes("department 'engineering'"))
        })

...
`modulo-7-exemplo-pratico-guardiao-familia/docs/02-criacao-board/10-agents/skills/qa/SKILL.md`

## Task

```powershell
python docs/02-criacao-board/10-agents/scripts/agent_orchestrator.py --agent qa --json
python docs/02-criacao-board/10-agents/scripts/agent_orchestrator.py --agent qa --claim --json
```

Elegível: (1) `agent_role=qa` com `board_status == Todo`; (2) fila **Ready for Test** (qualquer role). Status em `github-project-2-import.json` + Project #2 via `gh`.

Priorizar `release_blocker=True` e títulos com teste/e2e.

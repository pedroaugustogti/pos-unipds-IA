...
from lib.reviewer_pairs import CREATOR_ROLES, reviewer_for
...
RUNTIME_PATH = Path(__file__).resolve().parents[1] / "crew" / "output" / "agent_runtime.json"
BUG_THRESHOLD = 3
...
def resolve_agent_for_status(task: dict, status: str) -> str:
    ...
    creator = task.get("agent_role") or "backend"
...
def resolve_agent_for_event(task: dict, event: str) -> str:
    ...
    # Querem quem age apos o evento (owner do status destino)
    return resolve_agent_for_status(task, target)


def skill_impact_for_task(task: dict) -> dict[str, str]:
    role = task.get("agent_role") or "backend"
...
def record_bug(
    ...
    *,
    ...
    entry["history"].append({"at": _now(), "summary": (summary or "")[:500]})
    ...
    if not dry_run:
        ...
        # persiste contadores mesmo em dry_run (runtime), sem tocar board JSON
        save_runtime(rt)
    ...
    *,
    ...
    if bug_info and bug_info.get("blocker"):
        ...
        notification["dispatch"] = {
            ...
            "message": (
                f"Agente `{next_agent}` ocioso — chamar para iniciar/continuar "
                f"`{task_id}` em **{target}**."
            ),
        }

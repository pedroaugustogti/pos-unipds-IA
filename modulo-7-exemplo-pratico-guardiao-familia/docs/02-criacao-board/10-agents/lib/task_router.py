...
# Filas por papel (quando o agente atua na etapa do workflow, nao como criador de task QA)
ROLE_QUEUE_STATUSES: dict[str, frozenset[str]] = {
    "qa": frozenset({"Ready for Test"}),
...
def _claimable_statuses(agent: str) -> frozenset[str]:
    ...
    if agent in ROLE_QUEUE_STATUSES and agent == "qa":
        # agent-qa: (1) tasks com agent_role=qa ainda em Todo; (2) fila Ready for Test
        return CREATOR_CLAIMABLE_STATUSES | ROLE_QUEUE_STATUSES["qa"]
    if agent == "stores-release":
        return ROLE_QUEUE_STATUSES["stores-release"]
    if agent == "devops-cicd":
        # criador de tasks devops + fila de merge
...
def _in_stage_queue(row: dict, agent: str, board_status: str) -> bool:
    """Filas de workflow (QA / merge) independentes do agent_role do CSV."""
    if agent == "qa" and board_status == "Ready for Test":
        return True
    if board_status != "In Pull Request":
        return False
    track = row.get("track") or ""
    if agent == "stores-release" and track == "stores":
        return True
    if agent == "devops-cicd" and track != "stores":
        return True
    return False
...
def score_task(row: dict, agent: str, sprint_atual: int = 1) -> float:
    ...
    role_match = row["agent_role"] == agent or row.get("agent_role_secondary") == agent
    if role_match:
    ...
    if board_status == "Todo":
        s += 20
    if board_status == "Ready for Test" and agent == "qa":
        s += 40

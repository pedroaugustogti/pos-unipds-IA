#!/usr/bin/env python3
"""CLI para transições de status no GitHub Project #2."""

from __future__ import annotations

import importlib.util
from pathlib import Path

_bs = Path(__file__).resolve().parents[2] / "bootstrap.py"
_spec = importlib.util.spec_from_file_location("orch_bootstrap", _bs)
_mod = importlib.util.module_from_spec(_spec)
assert _spec and _spec.loader
_spec.loader.exec_module(_mod)
_mod.setup()

import argparse
import json
import sys
from pathlib import Path

from lib.paths import MODULE_ROOT  # noqa: E402
from lib.env_load import ensure_env  # noqa: E402

ensure_env()
from board_automation.board.board_client import update_project_status  # noqa: E402
from board_automation.board.local_board import get_local_status  # noqa: E402
from board_automation.board.task_router import load_tasks  # noqa: E402
from board_automation.board.task_status_workflow import (  # noqa: E402
    EVENT_TARGET,
    STATUSES,
    apply_event,
    can_transition,
    resolve_status,
    transition,
    transitions_for,
)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Transicao de status — JSON local + GitHub Project #2 (gh)"
    )
    parser.add_argument("--task", required=True, help="Task ID ex: T-P04-005")
    parser.add_argument("--event", choices=sorted(EVENT_TARGET.keys()), help="Evento de automacao")
    parser.add_argument("--status", help="Status destino explicito")
    parser.add_argument(
        "--current",
        default=None,
        help="Status atual (default: fields.Status no github-project-2-import.json)",
    )
    parser.add_argument("--kind", choices=["feature", "bug"], default="feature")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()

    tasks = load_tasks()
    task = next((t for t in tasks if t["id"] == args.task), None)
    if not task:
        raise SystemExit(f"Task {args.task} nao encontrada no mapa")

    current_raw = args.current or get_local_status(args.task) or task.get("board_status") or "Todo"
    current = resolve_status(current_raw)

    if args.event:
        target = apply_event(current, args.event, args.kind)
    elif args.status:
        target = transition(current, args.status, args.kind)
    else:
        parser.error("Informe --event ou --status")

    result = {
        "task_id": args.task,
        "kind": args.kind,
        "from": current,
        "to": target,
        "event": args.event,
        "transition_ok": can_transition(current, target, args.kind),
        "allowed_from_current": sorted(transitions_for(args.kind).get(current, set())),
        "statuses": list(STATUSES),
    }

    if not args.dry_run:
        board = update_project_status(args.task, task["title"], target, dry_run=False)
        result["board"] = board
    else:
        result["board"] = {"dry_run": True, "status": target, "local": True, "github": True}

    if args.json:
        print(json.dumps(result, ensure_ascii=False, indent=2))
    else:
        print(f"{result['from']} -> {result['to']}" + (f" (event: {args.event})" if args.event else ""))


if __name__ == "__main__":
    main()

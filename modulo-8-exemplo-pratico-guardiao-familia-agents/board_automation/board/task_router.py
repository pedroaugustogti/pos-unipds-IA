...
from board_automation.board.local_board import load_board, status_map
from lib.paths import MAP_CSV
...
def _enrich_from_board(rows: list[dict]) -> None:
    """Mescla qa/refinement/repo do github-project-*-import.json quando disponível."""
    try:
        board = load_board()
    ...
    for row in rows:
        ...
        if item.get("refinement"):
            row["refinement"] = item["refinement"]
        fields = item.get("fields") or {}
        repo = fields.get("Repo alvo") or item.get("repository")
        if repo:
            row["repo"] = str(repo)

...
_DRY_RUN = False
REVIEW_TEMPLATE = _LIB / "docs" / "templates" / "REVIEW_TEMPLATE.md"
...
def list_prs_pending_review(creator_role: str = "") -> str:
    """Lista tasks em Ready for Code Review / In Code Review no board JSON."""
    pending = []
    for t in load_tasks():
        if t.get("board_status") not in ("Ready for Code Review", "In Code Review"):
            continue
        role = t["agent_role"]

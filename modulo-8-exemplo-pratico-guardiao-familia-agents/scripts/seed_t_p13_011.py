#!/usr/bin/env python3
"""Cria T-P13-011 (LangGraph live) e remove T-P13-010 do Project."""
...
def main() -> int:
    for num, tid in (("76", "T-P13-010"),):
        r = _gh_run(
            ...
            "not planned",
            "--comment",
            f"Substituido por {TASK_ID} (LangGraph live — fluxo padrao do board).",
        )
    ...
    if old.get("item_id"):
        ...
        print("del item 010", r.returncode == 0)
...
## Orquestração
Executado pelo **LangGraph** (`python scripts/langgraph_run.py --task {TASK_ID} --mode live --from-zero`) — fluxo padrão de todas as atividades do board.

## Aceite
1. H1 + title atualizados e versionados
2. Playwright na issue
3. Comentários thought/action/observation com modelo+tokens (`select_model`)
4. Parar em **In Pull Request** (HITL, sem merge)
"""
    r = _gh_run(
        "issue",
        "create",
    ...
    board["items"] = [
        ...
        if i.get("id") not in ("T-P13-008", "T-P13-009", "T-P13-010")
    ]
    board["items"].append(
        {
            "id": TASK_ID,
            ...
            "fields": {
                ...
                "Priority Rank": 0,
                "Repo alvo": REPO,
                "Refinamento": "Hero H1+title via LangGraph. Playwright. HITL pre-merge.",
    ...
    lines = [
        ...
        if not ln.startswith(("T-P13-008,", "T-P13-009,", "T-P13-010,"))
    ]

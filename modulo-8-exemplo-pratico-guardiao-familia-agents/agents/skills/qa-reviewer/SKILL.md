...
description: >-
  Revisor de testes QA Author. Pareado com qa. Valida cobertura cenarios criticos,
  finaliza PR e board.
---

# Revisor QA Author — par de `agents/skills/qa`
...
| Board status | Nó | Papel (`qa-reviewer`) |
|--------------|-----|------------------------|
...
- [ ] Testes rodam localmente (`agents/qa-gate/scripts/local_e2e_smoke.py` / `agents/qa-gate/scripts/local_e2e_stack.ps1`)
- [ ] Sem flaky nao documentado
- [ ] Gaps listados no PR
- [ ] Evidencias (report JSON / screenshots) na issue quando E2E mobile
...
| PR fora do escopo do par (feature prod) | `changes_requested` + agente sugerido |

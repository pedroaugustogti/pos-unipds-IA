...
from lib.orchestrator.event_orchestrator import list_idle_agents, load_runtime, save_runtime, set_agent_state  # noqa: E402
from lib.gateway.handoff import load_handoff  # noqa: E402
from lib.paths import PROMPTS_DIR  # noqa: E402
from lib.core.react_policy import max_iterations_for  # noqa: E402
...
def build_prompt_bundle(job: dict) -> Path:
    ...
    role = job["role"]
    task_id = job["task_id"]
    agent_md = MODULE_MODULE_ROOT / job.get(
        "agent_prompt",
        agent_prompt_path(role).relative_to(MODULE_ROOT).as_posix(),
    )
    handoff = load_handoff(task_id) or {}
    skill = skill_path(role)
    ...
    parts = [
        ...
        f"   `agents/00-runtime/output/dispatch/results/{job['job_id']}.json`",
        "3. Rode (a partir do modulo agents):",
        f"   `python agents/00-orchestration/scripts/worker/complete_dispatch.py --job {job['job_id']}`",
        "",
        ...
        f"`python agents/00-orchestration/scripts/worker/worker_run.py --complete --job {job['job_id']}`",
        f"`python agents/00-orchestration/scripts/cli/gateway_cli.py --task {task_id} --event <evento> ...`",
        "",
        "## Handoff",
        "```json",

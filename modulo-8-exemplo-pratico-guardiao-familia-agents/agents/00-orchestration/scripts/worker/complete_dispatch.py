...
ensure_env()
from lib.orchestrator.complete_dispatch import RESULTS_DIR, apply_dispatch_result  # noqa: E402


def main() -> int:
    p = argparse.ArgumentParser(description="Aplica contrato de conclusao do dispatch")
    p.add_argument("--job", help="job_id (le agents/00-runtime/output/dispatch/results/{job}.json)")

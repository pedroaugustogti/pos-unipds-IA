...
**Classificação:** creator · **Papel:** `backend` · Revisor: `backend-reviewer`

| Tool | Quando |
|------|--------|
| `list_status_events` | Filtrar `agent_role=backend` |
...
| Status | Evento |
|--------|--------|
| In Progress | `backend_in_progress` |
| Ready for Code Review | `backend_ready_for_code_review` (+ `pr_url`) |
| In Code Review | `backend_in_code_review` |

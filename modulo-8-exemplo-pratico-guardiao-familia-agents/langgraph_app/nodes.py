...
from lib.task_action_history import build_agent_observation
...
def route_task(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if _mode(state) != "dry_run":
        ...
        if fresh:
            ...
            task = fresh
    tier = select_model(task, purpose="route", role=task.get("agent_role"))
    creator = normalize_creator_role(str(task.get("agent_role") or "backend"))
    return {
        "title": task.get("title") or state.get("title") or "",
        "board_status": status,
        # agent_role no state = creator da task (CSV); acting muda no apply
        "agent_role": creator,
        "model_tier": tier,
        "messages": list(state.get("messages") or []) + [f"route: status={status} creator={creator}"],
        "steps": int(state.get("steps") or 0) + 1,
        "react_trace": [
            {"thought": "Roteamento", "action": "route_task", "observation": status}
        ],
    }
...
def load_context(state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "messages": list(state.get("messages") or []) + ["context: ok"],
        "steps": int(state.get("steps") or 0) + 1,
        "react_trace": [{"thought": "Contexto", "action": "load_context", "observation": "ok"}],
    }
...
def _implement_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    dry = _dry(state)
    mode = _mode(state)
    ws = MODULE_ROOT / "crew" / "output" / "langgraph" / "workspace" / tid
    ...
    if not dry:
        try:
            from lib.site_hero_work import apply_hero_commit, is_site_hero_task
    ...
    content = (
        ...
        f"Mode: {mode}\n"
        f"Site hero: {impl_meta or 'n/a'}\n"
        f"Gerado pelo no implement (orquestracao padrao).\n"
    )
    path = None
    if not dry:
    ...
    try:
        prompt = (
            ...
            f"Detalhe site={impl_meta.get('sha') if impl_meta else 'artefato local'}."
        )
    ...
    if not dry:
        tools.append_task_action(
            ...
            observation=build_agent_observation(
                ...
                extra=hist_extra,
                detail=f"site_ok={site_ok}; {usage_line}",
                ok=bool(site_ok or path),
            ),
            dry_run=False,
            record_history=True,
...
def _qa_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not dry:
        try:
            ...
            from lib.qa_playwright import format_qa_issue_comment
            from lib.site_hero_work import is_site_hero_task, run_hero_qa
...
def _apply_decision(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if event == "open_pr":
        kwargs["pr_url"] = f"https://example.com/langgraph/{state['task_id']}"

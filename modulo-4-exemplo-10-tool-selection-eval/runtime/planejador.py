...
def _ferramentas_da_percepcao(percepcao: str) -> set:
    for linha in percepcao.split("\n"):
        if linha.startswith("Ferramentas ja utilizadas:"):
            parte = linha.split(":", 1)[1].strip()
            if parte:
                return {nome.strip() for nome in parte.split(",") if nome.strip()}
    return set()


def _ferramentas_ja_usadas(historico: list, percepcao: str) -> set:
    return _ferramentas_do_historico(historico) | _ferramentas_da_percepcao(percepcao)


def _reparar_json_llm(conteudo: str) -> dict:
    texto = (conteudo or "").strip()
    if not texto:
        raise json.JSONDecodeError("vazio", "", 0)
    if texto.startswith("```"):
...
def _inferir_tool_prioritaria(entrada: str, percepcao: str, usadas: set, ordem: list) -> str | None:
    texto = f"{entrada}\n{percepcao}".lower()
    coleta_completa = {"consultar_metricas", "buscar_logs", "historico_deploys", "buscar_issues"}

    if coleta_completa.issubset(usadas) and "relatorio_incidente" in ordem and "relatorio_incidente" not in usadas:
        return "relatorio_incidente"

    if re.search(r"\b(issues?\s+abert|buscar\s+issues|tickets?\s+abert|ha\s+issues)\b", texto):
        if "buscar_issues" in ordem and "buscar_issues" not in usadas:
            return "buscar_issues"

    if not usadas and re.search(r"\b(deploy|release|rollout)\b", texto):
        if re.search(r"\bissues?\b", texto) and "buscar_issues" in ordem:
            return "buscar_issues"
        if "historico_deploys" in ordem:
            return "historico_deploys"
...
def chamar_llm(percepcao: str, contratos: dict, historico: list = None, entrada: str = "") -> tuple:
    ...
    timeout_seg = float(os.environ.get("RUNTIME_LLM_TIMEOUT_SEC", "120"))
    ...
    try:
        futuro = executor.submit(_chamar_api)
        resposta = futuro.result(timeout=timeout_seg)
    except FuturesTimeoutError:
        print(f"  [planejador] LLM timeout ({timeout_seg}s), usando mock contextual")
        executor.shutdown(wait=False, cancel_futures=True)
        plano = planejador_mock(percepcao, contratos, historico, entrada)
        plano["_modo"] = "mock_timeout"
        return plano, _TOKENS_ZERO.copy()
    except Exception as erro:
        print(f"  [planejador] LLM indisponivel ({erro}), usando mock contextual")
        executor.shutdown(wait=False, cancel_futures=True)
        plano = planejador_mock(percepcao, contratos, historico, entrada)
        plano["_modo"] = "mock_fallback"
        return plano, _TOKENS_ZERO.copy()
    finally:
        executor.shutdown(wait=False, cancel_futures=True)
    ...
    try:
        plano = _reparar_json_llm(resposta.choices[0].message.content)
        plano["_modo"] = "llm"
        return _injetar_campos_arquitetura(plano, contratos, _ferramentas_do_historico(historico)), uso_tokens
    except (json.JSONDecodeError, IndexError, TypeError):
        print("  [planejador] resposta JSON invalida, usando mock contextual")
        plano = planejador_mock(percepcao, contratos, historico, entrada)
        plano["_modo"] = "mock_fallback"
...
def planejador_mock(percepcao: str, contratos: dict, historico: list = None, entrada: str = "") -> dict:
    ...
    habilidades = contratos.get("habilidades", {}).get("habilidades", [])
    historico = historico or []
    usadas = _ferramentas_ja_usadas(historico, percepcao)
    ordem = _ordem_ferramentas(contratos)
    formato = _formato_arquitetura(contratos)
    inclui_raciocinio = "raciocinio" in formato

    prioritaria = _inferir_tool_prioritaria(entrada, percepcao, usadas, ordem)
    if prioritaria:
        habilidade = next((hab for hab in habilidades if hab["nome"] == prioritaria), {})
        argumentos = montar_argumentos_mock(habilidade, historico, entrada)
        plano = {
            "proxima_acao": "CHAMAR_FERRAMENTA",
            "nome_ferramenta": prioritaria,
            "argumentos_ferramenta": argumentos,
            "criterio_sucesso": f"{prioritaria} executado com sucesso",
        }
        if inclui_raciocinio:
            plano["raciocinio"] = (
                f"Intencao detectada ou coleta completa. Proximo passo: {prioritaria}."
            )
